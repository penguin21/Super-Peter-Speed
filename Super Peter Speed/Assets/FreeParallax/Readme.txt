FreeParallax 1.0, by Jeff Johnson, Founder of Digital Ruby, LLC (c) 2015
...
I reallly appreciate feedback, please email me at freeparallax@digitalruby.com if you have any questions, feedback or bug reports.
...
- Change the speed based on your game rquirements. If you are controlling a character like a platformer, set the speed to 0 and change the speed in code based on character movement (see the FreeParallaxDemo.cs file for an example).
- Set the size of the elements list on the script to the number of layers in your parallax. Notice that the Unity editor expans
  the list to the size you typed in (just type in the size and press ENTER).
...
- Draw a game object from the hiararchy into the game object slots in the list
...
- Setup the repositioning logic. The reposition mode contains 4 possible options:
   Wrap Anchor Bottom : The layer will be seamless and fill the entire width and be anchored to the bottom of the screen
   Wrap Anchor Top : The layer will be seamless and fill the entire width and be anchored to the top of the screen
...
- Set the scale height (as a percentage of screen height)
Note: For full size wrapping elements, 1 is a good value as it ensures the graphic is the same height as the screen
Note: For wrapping elements that are smaller than the screen height, this value can be set to something smaller than 1, like a road or river
