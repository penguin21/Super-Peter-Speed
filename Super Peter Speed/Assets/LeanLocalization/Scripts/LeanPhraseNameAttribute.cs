	...
	public class LeanPhraseNameDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (GUI.Button(right, "List") == true)
			{
				...
				for (var i = 0; i < LeanLocalization.AllLocalizations.Count; i++)
				{
					var localizaion = LeanLocalization.AllLocalizations[i];
					
					for (var j = 0; j < localizaion.Phrases.Count; j++)
					{
						var phraseName = localizaion.Phrases[j].Name;
